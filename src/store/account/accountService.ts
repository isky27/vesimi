...
const getSingleAddressApi = async (userData: any, userToken: { headers: { Authorization: string } }): Promise<ApiResponse> => {
  try {
    const response: ApiResponse = await axios.get(`user/shipping/detail?id=${userData?.addressId}`, userToken);
    return response;
  } catch (error: any) {
    throw error;
  }
...
const updateAddressApi = async (userData: any, userToken: { headers: { Authorization: string } }): Promise<ApiResponse> => {
  try {
    const response: ApiResponse = await axios.post(`user/shipping/update`, userData, userToken);
    return response;
  } catch (error: any) {
    throw error;
  }
